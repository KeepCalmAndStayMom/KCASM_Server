...
public class ApiPatientTasks {
    ...
    private void showTasksCategory() {
        get("", (request, response) -> {
            ...
            return PatientTasksLinks.tasksMenu(patientId, "patient");
        }, gson::toJson);
    }
    ...
    private void getSingleTask(String taskCategory) {
        get("/:task_id", (request, response) -> {
            ...
            if(query != null) {
                query.put("links", PatientTasksLinks.patientSingleTaskLinks(patientId, taskId, (int) query.get("medic_id"), taskCategory));
                response.status(200);
                response.type("application/json");
                return query;
            }
